...
/*! Gitspace 0.0.2 //// MIT Licence //// http://gitspace.richplastow.com/ */
(function(global) {
...
DB = (function() {
  ...
  DB.prototype.update = function(body) {
    var M, i, k, match, v;
    M = '/gitspace/src/DB.litcoffee DB::list()\n  ';
    oo('update');
...
GitSpace = (function() {
  ...
  GitSpace.prototype.start = function(yy) {
    ...
    wsServer.broadcast = (function(_this) {
      return function(data) {
        var client, err, error1, j, len, ref, results;
        if (oo.S !== oo.type(data)) {
          data = JSON.stringify(data);
        }
        ref = wsServer.clients;
        results = [];
        for (j = 0, len = ref.length; j < len; j++) {
          client = ref[j];
          oo(data);
          try {
            ...
            err = error1;
            results.push(oo('broadcast error:', err));
          }
        }
        return results;
      };
      ...
      return function(ws) {
        ...
        return ws.on('message', function(data, flags) {
          ...
          if ('POST ' === data.substr(0, 5)) {
            ...
            return _this[oo._]._servers.ws.broadcast(_this[oo._]._db.list);
          } else if ('hi' === data) {
      ...
      request.on('end', (function(_this) {
        return function() {
          ...
          if (_this[oo._]._servers.ws.clients.length) {
            _this[oo._]._servers.ws.broadcast(_this[oo._]._db.list);
          }
...
DB.FileSystem = (function(superClass) {
  ...
  FileSystem.prototype.xx = function(yy) {
    var M;
    M = '/gitspace/src/DB/FileSystem.litcoffee DB::xx()\n  ';
    return yy = oo.vArg(M, yy, 'yy <number>', 123);
  };
...
DB.Memory = (function(superClass) {
  ...
  Memory.prototype.xx = function(yy) {
    var M;
    M = '/gitspace/src/DB/Memory.litcoffee DB::xx()\n  ';
    return yy = oo.vArg(M, yy, 'yy <number>', 123);
  };
